...
# Sample data
SOURCES = ["Quick Mart", "Fresh Foods", "Speed Supplies", "Urban Goods", "Prime Picks"]
ITEMS = ["Electronics Bundle", "Kitchen Set", "Book Collection", "Fitness Kit", "Garden Tools", "Office Pack"]
DRIVERS = ["John Smith", "Maria Garcia", "Ahmed Khan", "Lisa Chen", "Carlos Rodriguez"]
BUYERS = ["Alice Johnson", "Bob Williams", "Carol Davis", "David Martinez", "Emma Wilson"]
ADDRESSES = [
    "123 Main St, Apt 4B",
    "456 Oak Avenue",
...
async def create_sample_order(order_service: OrderService, status: OrderStatus, days_ago: int = 0):
    ...
    order = Order(
        ...
        source_price=round(random.uniform(8.0, 15.0), 2),
    ...
    if status in [OrderStatus.SOURCE_ACCEPTED, OrderStatus.BUYER_ACCEPTED,
                  OrderStatus.PREPARING, OrderStatus.READY, OrderStatus.DISPATCHED,
                  OrderStatus.IN_TRANSIT, OrderStatus.DELIVERED]:
        await order_service.source_respond(
            order_id,
            accept=True,
    ...
    if status in [OrderStatus.BUYER_ACCEPTED, OrderStatus.PREPARING, OrderStatus.READY,
...
async def generate_test_data():
    ...
    try:
        ...
        for _ in range(pending_count):
            await create_sample_order(order_service, OrderStatus.PENDING_SOURCE)
